...
PG_FUNCTION_INFO_V1(reservoir_sampler_tpch_3gb);
Datum
reservoir_sampler_tpch_3gb(PG_FUNCTION_ARGS)
{
    ...
    TupleDescInitEntry(tupdesc, (AttrNumber) 1, "l_discount", NUMERIC4OID, -1, 0);
    ...
    for(row = 0; row < SPI_processed; row++){
        ...
        int attnum3 = SPI_fnumber(SPI_tuptable->tupdesc, "l_discount");
