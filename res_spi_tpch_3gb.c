...
Datum
reservoir_sampler_tpch_3gb(PG_FUNCTION_ARGS)
{
    int ret;
    int row;
    Tuplestorestate *tupstore;
...
//     //TupleDescInitEntry(tupdesc, (AttrNumber) 1, "b", INT8OID, -1, 0);
//     //TupleDescInitEntry(tupdesc, (AttrNumber) 1, "c", INT8OID, -1, 0);
//     //TupleDescInitEntry(tupdesc, (AttrNumber) 1, "a", INT4ARRAYOID, -1, 0);
//     TupleDescInitEntry(tupdesc, 1, "b", INT8OID, -1, 0);
//     TupleDescInitEntry(tupdesc, 2, "c", INT8OID, -1, 0);
//     TupleDescInitEntry(tupdesc, 3, "a", INT4ARRAYOID, -1, 0);
//     //TupleDescInitEntry(tupdesc, (AttrNumber) 1, "a", INT8OID, -1, 0);
//     oldcontext = MemoryContextSwitchTo(rsinfo->econtext->ecxt_per_query_memory);
//     tupstore = tuplestore_begin_heap(true, false, work_mem);
//     rsinfo->setResult = tupstore;
//     tupdesc = CreateTupleDescCopy(SPI_tuptable->tupdesc);
...
//     int b;
//     int c;
//     Datum *elems = (Datum *) palloc(sampleSize * sizeof(Datum));
//     for(row = 0; row < SPI_processed; row++){
     
//         int attnum1 = SPI_fnumber(SPI_tuptable->tupdesc, "b");
//         int attnum2 = SPI_fnumber(SPI_tuptable->tupdesc, "c");
//         int attnum3 = SPI_fnumber(SPI_tuptable->tupdesc, "a");
//         char* value1 = SPI_getvalue((SPI_tuptable->vals)[row], SPI_tuptable->tupdesc, attnum1);
//         char* value2 = SPI_getvalue((SPI_tuptable->vals)[row], SPI_tuptable->tupdesc, attnum2);
//         char* value3 = SPI_getvalue((SPI_tuptable->vals)[row], SPI_tuptable->tupdesc, attnum3);
//         char *current_group = strcat(value1, ",");
...
//                 Datum values[3];
//                 bool nulls[3];
//                 values[0] = Int32GetDatum(atoi(value1));
//                 values[1] = Int32GetDatum(atoi(value2));
//                 //values[2] = elems;
//                 values[2] = PointerGetDatum(construct_array(elems, sampleSize, INT4OID, sizeof(int32), true, 'i'));
//                 nulls[0] = false;
//                 nulls[1] = false;
//                 nulls[2] = false;
//                 HeapTuple tuple = heap_form_tuple(tupdesc, values, nulls);
...
//                 HeapTuple tuple = SPI_tuptable->vals[row];
//                 reservoir[poscnt] = tuple;
//                 elems[poscnt] = Int32GetDatum(atoi(value3));
//                 poscnt ++;
//         } else {
//             int pos = rand() % (poscnt+1);
//             if (pos < sampleSize){
...
//                 HeapTuple tuple = SPI_tuptable->vals[row];
//                 reservoir[pos] = tuple; 
//                 elems[pos] = Int32GetDatum(atoi(value3));
//             }
//             poscnt ++;
//         }
...
//     Datum values2[3];
//     bool nulls2[3];
//     values2[0] = Int32GetDatum(b);
//     values2[1] = Int32GetDatum(c);
//     //values2[2] = elems;
//     values2[2] = PointerGetDatum(construct_array(elems, sampleSize, INT4OID, sizeof(int32), true, 'i'));
//     nulls2[0] = false;
//     nulls2[1] = false;
//     nulls2[2] = false;
//     HeapTuple tuple = heap_form_tuple(tupdesc, values2, nulls2);
//     tuplestore_puttuple(tupstore, tuple);
